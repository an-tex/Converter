  ...
  override def leaveContainerTree(scope: TreeScope)(container: ContainerTree): ContainerTree =
    ...
    if (scope.stack.exists { case mod: ModuleTree => mod.isNative; case _ => false })
      ...
      container.withMembers(container.members.flatMap {
        case member: Tree with HasCodePath if !nameConflict(member.name) =>
          val generatedMembers: IArray[Tree] =
            member match {
              case RewrittenTypeUnion(rewritten) =>
                genMethodsFromTypeUnion(scope, rewritten)
              case cls: ClassTree =>
                genMethodsForTrait(scope, cls)
              case _ => Empty
            }

          generatedMembers match {
            case Empty => IArray(member)
            case some =>
              IArray(
                member,
                ModuleTree(Empty, member.name, Empty, some, NoComments, member.codePath, isOverride = false),
              )
          }
        case otherwise => IArray(otherwise)
      })
    }

  private def genMethodsForTrait(scope: TreeScope, cls: ClassTree) = {
